...
import (
	...
	"github.com/avila-r/ego/failure/id"
	"github.com/avila-r/ego/failure/modifier"
...
func (c *ErrorClass) Of(message string, v ...any) *Failure {
	return c.New(message, v...)
}

func (c *ErrorClass) New(message string, v ...any) *Failure {
	return Builder(c).
		Message(message, v...).
		Build()
}

func (c *ErrorClass) Blank() *Failure {
	return Builder(c).
		Build()
}

func (c *ErrorClass) Wrap(err error, message string, v ...any) *Failure {
	return Builder(c).
		Message(message, v...).
		Cause(err).
		Build()
...
func (c *ErrorClass) From(err error) *Failure {
	return Builder(c).
		Cause(err).
		Build()
}
